...
from utils.get_trunk import get_trunk
from utils.get_weight import get_weight

responses = get_responses()
trunk = get_trunk(dict(responses))
weights = []
for child in responses[trunk]['children']:
    weight = get_weight(child, responses)
    if len(weights) > 1 and weight not in weights:
        print("LIKELY IMBALANCE DETECTED!! {} != {}".format(weight, weights[0]))
        debugger(child, responses)
