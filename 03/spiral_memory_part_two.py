...
"""
import sys
from utils import next_coords

x, y = 0, 0
spiral = {x: {y: 1}}
for i in range(2, int(sys.argv[1]) + 1):
    x, y = next_coords.next_coords(spiral, x, y)
    value = 0

    for j in range(-1, 2):
        test_x = x + j
        if test_x in spiral:
            if (y - 1) in spiral[test_x]:
                value += spiral[test_x][(y - 1)]
            if y in spiral[test_x]:
                value += spiral[test_x][y]
            if (y + 1) in spiral[test_x]:
                value += spiral[test_x][(y + 1)]
